...
create function private.process_paynow_update(
  p_reference text,
...
declare
  ...
  v_paid boolean := v_normalized in ('paid', 'awaiting delivery', 'delivered');
...
begin
  ...
  where booking_id = v_booking.id and provider = 'paynow'
  ...
  values (null, 'booking', v_booking.id::text, 'paynow_status_received',
...
revoke all on function private.process_paynow_update(text, text, numeric, text, jsonb)
  from public, anon, authenticated;
grant execute on function private.process_paynow_update(text, text, numeric, text, jsonb) to service_role;
grant usage on schema private to service_role;

create function public.process_paynow_update(
  p_reference text, p_provider_reference text, p_amount numeric, p_status text, p_payload jsonb
) returns jsonb
language sql
security invoker
set search_path = ''
as $$ select private.process_paynow_update(p_reference, p_provider_reference, p_amount, p_status, p_payload) $$;

revoke all on function public.process_paynow_update(text, text, numeric, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.process_paynow_update(text, text, numeric, text, jsonb) to service_role;
